'''
Sample 10 - FAV Enunciado ZPL (page 25)
'''

import os, csv, tkinter as tk, socket
...
'''         
            if enviar_check.get() == 1:
                ...
                status_label.config(text=f'A enviar etiqueta {i + 1} de {total} para a impressora {ip}:{porta}')
                janela.update_idletasks()

                with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as zebra_socket:
                    try:
                        zebra_socket.connect((ip, int(porta)))
                        zebra_socket.sendall(zpl.encode('utf-8'))

                        status_label.config(text=f'✓ Etiqueta {i + 1} de {total} enviada com sucesso.')
                        janela.update_idletasks()
                    except Exception as e:
                        messagebox.showerror("Erro", f"Erro ao enviar etiqueta {i + 1}: {nome}\n{e}")
                        return  
...
ip_entry.insert(0, "192.168.1.200")
...
tk.Checkbutton(
    frame, 
    text="Enviar diretamente para a impressora", 
    variable=enviar_check, 
    bg = frame["bg"], 
    fg="white",
    selectcolor=frame["bg"]
...
for widget in frame.winfo_children():
    if isinstance(widget, tk.Label):
        ...
        widget.configure(bg=DarkTheme.DARK_BG, fg=DarkTheme.TEXT_COLOR, selectcolor=DarkTheme.DARK_BG)
